...
# def SSE_f_fn(model, x, u):
#     return torch.sum((torch.sum(torch.diagonal(vmap(hessian(model))(x), 0, -2, -1), -1) + u + (ndims*4*pi**2 - 1)*torch.prod(torch.sin(2*pi*x), -1, True))**2)
...
# Parameters
directory = 'models/Adam vs LBFGS'
ndims = [2]
bounds = [0, 1]
