...
def SSE_f_fn(model, x, u):
    ...
    I = torch.eye(x.size(1), device=device)
    d2 = torch.diagonal(vmap(lambda I: torch.autograd.grad(d1, x, grad_outputs=torch.ones_like(d1)*I, create_graph=True)[0], 1)(I), 0, 0, 2)
...
for ndims, N, L, activation, num_points, use_sobol, batch_size, lr, lr_scheduler in itertools.product(ndims, N, L, activation, num_points, use_sobol, batch_size, lr, lr_scheduler):
    name = f'{ndims},{bounds},{N},{L},{activation},{num_points},{use_sobol},{batch_size},{lr},{lr_scheduler},torch.autograd.grad+vmap'
