...
laplacian = lambda u, x: torch.sum(torch.diagonal(vmap(hessian(u))(x), 0, -2, -1), -1)


def SSE_f_fn(model, x, u):
    return torch.sum((laplacian(model, x) + u + (ndims*4*pi**2 - 1)*torch.prod(torch.sin(2*pi*x), -1, True))**2)
...
def train(model, optimizer, num_points, use_sobol=True, batch_size=1024, max_time=None, MSE_f=0, i=0):
    ...
    while i < num_points and time() - start_time < max_time:
        ...
        if use_sobol:
            x = (sobolengine.draw(n)*(bounds[1] - bounds[0]) + bounds[0]).to(device)
        else:
            x = (torch.rand(n, ndims, device=device)*(bounds[1] - bounds[0]) + bounds[0])
        pred = model(x)
...
def test(model, num_points, batch_size=1):
    ...
    for i in range(0, num_points, batch_size):
        x = (torch.rand(min(batch_size, num_points - i), ndims, device=device)*(bounds[1] - bounds[0]) + bounds[0])
...
for ndims, N, L, activation, num_points, use_sobol, batch_size, lr, lr_scheduler in itertools.product(ndims, N, L, activation, num_points, use_sobol, batch_size, lr, lr_scheduler):
    name = f'{ndims},{bounds},{N},{L},{activation},{num_points},{use_sobol},{batch_size},{lr},{lr_scheduler},torch.func.hessian+vmap'
